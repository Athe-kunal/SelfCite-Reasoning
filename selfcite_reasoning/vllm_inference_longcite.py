...
class LongCiteModel(LLM):
    ...
    def chat(
        ...
        outputs = self.generate(
            sampling_params=generation_params, prompt_token_ids=[input_ids]
        )
    ...
    def query_longcite(
        ...
        def get_citations(statement, sents):
            ...
            spans = sum(
                [
                    re.findall(r"\[([0-9]+\-[0-9]+)\]", c_text, re.DOTALL)
            ...
            for i, s in enumerate(spans):
                try:
                    ...
                    if (
                        len(merged_citations) > 0
                        and st == merged_citations[-1]["end_sentence_idx"] + 1
                    ):
                        ...
                        merged_citations.append(
                            {
                                ...
                                "end_char_idx": sents[ed]["end"],
                                "cite": "".join(
                                    [x["content"] for x in sents[st : ed + 1]]
                                ),
                            }
                        )
                except:
        ...
        def postprocess(answer, sents, splited_context):
            ...
            while True:
                ...
                if len(statement.strip()) > 5:
                    res.append({"statement": statement, "citation": []})
                ...
                if len(statement.strip()) > 0:
                    ...
                    res.append({"statement": statement, "citation": citations})
                    c_str = "".join(
                        [
                            "[{}-{}]".format(
                    ...
                    new_answer += (
                        f"<statement>{statement}<cite>{c_str}</cite></statement>"
                    )
                else:
                    res.append(
                        {
            ...
            return {
                ...
                "statements_with_citations": [
                    x for x in res if x["citation"] is not None
                ],
